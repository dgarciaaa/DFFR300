...
DEFAULT_LECTURES_DIR = REPO_ROOT / "lectures"
...
def _cargar_usuarios_desde_texto(config_path=None):
    ...
    if not path.exists():
        ...
        print("💡 Crea 'scripts_canvas/Key.txt' con formato: profesor|token|course_id")
        return {}
...
unidades = {
    "UNIDAD I: MECÁNICA DE LA PARTÍCULA EN UNA DIMENSIÓN": [1, 2, 3, 4, 5, 6],
    "UNIDAD II: MECÁNICA DE FLUIDOS": [7, 8],
    "UNIDAD III: TEMPERATURA, CALOR Y TERMODINÁMICA": [9, 10, 11, 12],
    "UNIDAD IV: ELECTRICIDAD Y MAGNETISMO": [13, 14, 15, 16],
}

semanas_por_unidad = {
    1: "Semana 01 | Magnitudes, unidades y vectores",
    2: "Semana 02 | Cinemática de la partícula",
    3: "Semana 03 | Cinemática con aceleración constante",
    4: "Semana 04 | Interacción y leyes del movimiento",
    5: "Semana 05 | Aplicaciones de las leyes de Newton",
    6: "Semana 06 | Trabajo, energía, impulso y momentum",
    7: "Semana 07 | Estática de fluidos",
    8: "Semana 08 | Fluidos en movimiento y propiedades interfaciales",
    9: "Semana 09 | Temperatura y descripción térmica",
    10: "Semana 10 | Calor, energía interna y calorimetría",
    11: "Semana 11 | Primera ley y transferencia de calor",
    12: "Semana 12 | Segunda ley y entropía",
    13: "Semana 13 | Carga, fuerza y campo eléctrico",
    14: "Semana 14 | Potencial, capacitancia y condensadores",
    15: "Semana 15 | Corriente, resistencias y circuitos RC",
    16: "Semana 16 | Magnetismo e integración del curso",
}
...
def _partes_semana(numero_semana):
    ...
    Cada semana del plan vigente tiene dos sesiones (P1 y P2).
    """
    return ["P1", "P2"]
...
def select_user(nombre_usuario):
    ...
    Args:
        nombre_usuario: Nombre configurado en Key.txt o un token personalizado
...
def ver_cursos(filtro_codigo="DFFR300"):
    """
    Muestra todos los cursos disponibles para el usuario conectado
    
    Args:
        filtro_codigo: Código para filtrar cursos (por defecto "DFFR300")
...
def _extraer_numero_semana(titulo):
    ...
    Extrae el número de títulos tipo:
      Semana 12
      Semana 12 | Segunda ley y entropía
      Semana 012
...
def subir_contenido(numero_semana, course_id=None, test_mode=False,
                    titulo_semana=None, titulos_pdf=None):
    """
    ...
    Args:
        ...
        titulo_semana: Título visible opcional para el encabezado de la semana.
        titulos_pdf: Diccionario opcional con títulos visibles para "P1" y/o "P2".
                     Los nombres de los archivos locales no cambian.
    
    Returns:
        El módulo creado o actualizado
        
    Ejemplo:
        Canvas_Key.subir_contenido(11)  # Primera ley y transferencia de calor
        Canvas_Key.subir_contenido(12)  # Segunda ley y entropía
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
        print("💡 Primero ejecuta: Canvas_Key.select_user('TuNombre')")
    ...
    # Resolver títulos visibles sin alterar los nombres de archivos locales.
    semana_titulo = titulo_semana.strip() if titulo_semana else semanas_por_unidad[numero_semana]
    if not semana_titulo:
        print("❌ Error: titulo_semana no puede estar vacío")
        return None
    if titulo_semana:
        numero_en_titulo = _extraer_numero_semana(semana_titulo)
        if numero_en_titulo is None:
            semana_titulo = f"Semana {numero_semana:02d} | {semana_titulo}"
        elif numero_en_titulo != numero_semana:
            print(
                f"❌ Error: el título indica Semana {numero_en_titulo}, "
                f"pero se está subiendo la semana {numero_semana}"
            )
            return None

    titulos_pdf = titulos_pdf or {}
    partes_invalidas = set(titulos_pdf) - set(_partes_semana(numero_semana))
    if partes_invalidas:
        print(f"❌ Error: partes no válidas en titulos_pdf: {', '.join(sorted(partes_invalidas))}")
        return None

    titulos_visibles = {}
    for parte in _partes_semana(numero_semana):
        titulo = titulos_pdf.get(parte)
        if titulo is not None:
            titulo = titulo.strip()
            if not titulo:
                print(f"❌ Error: el título visible de {parte} no puede estar vacío")
                return None
        titulos_visibles[parte] = titulo
    
    # Formatear número de semana con ceros a la izquierda (para archivos)
    semana_str = f"{numero_semana:02d}"

    # Resolver y validar todos los archivos antes de conectarse al curso o
    # modificar cualquier elemento en Canvas.
    partes = _partes_semana(numero_semana)
    archivos_semana = [f"semana{semana_str}-{parte}.pdf" for parte in partes]
    for parte, archivo in zip(partes, archivos_semana):
        if titulos_visibles[parte] is None:
            titulos_visibles[parte] = archivo
    semana_dir = LECTURES_BASE_PATH / f"semana{semana_str}"
    files_to_upload = [semana_dir / archivo for archivo in archivos_semana]
    archivos_faltantes = [path for path in files_to_upload if not path.is_file()]
    if archivos_faltantes:
        print("❌ Prevalidación fallida: no se modificará Canvas.")
        for path in archivos_faltantes:
    ...
    # Subir primero los PDF a Canvas Files. La estructura anterior del módulo
    # solo se reemplaza cuando todos los uploads terminaron correctamente.
    archivos_subidos = []
    print("\n0. Pre-cargando archivos sin modificar el módulo...")
    try:
        for file_path in files_to_upload:
            uploaded_file = course.upload(str(file_path))
            file_id = uploaded_file[1]['id']
            archivos_subidos.append((file_path, file_id))
            print(f"  ✓ {file_path.name} subido (ID: {file_id})")
    except Exception as exc:
        print(f"❌ Falló la carga previa; la estructura del módulo no fue reemplazada: {exc}")
        return None
    ...
    if titulo_semana or titulos_pdf:
        print("Títulos visibles:")
        for parte in partes:
            print(f"  {parte}: {titulos_visibles[parte]}")
    print("=" * 80)
    ...
    for (file_path, file_id), parte in zip(archivos_subidos, partes):
        file_name = file_path.name
        titulo_visible = titulos_visibles[parte]
        print(f"  📄 {file_name}...")

        # Agregar el archivo ya cargado como item del módulo con indentación 2.
        main_module.create_module_item(
            module_item={
                'type': 'File',
                'content_id': file_id,
        ...
        print(f"    ✓ Agregado como '{titulo_visible}' en posición {insert_position + 1 + uploaded_count}")
        uploaded_count += 1
...
def eliminar_semana(numero_semana, course_id=None, test_mode=False):
    ...
    Borra:
      ...
      - No elimina archivos de Canvas Files, porque pueden estar referenciados
        desde otros módulos.
    ...
    # Se conservan los archivos de Canvas Files: pueden estar referenciados por
    # otros módulos (incluido el de producción cuando se opera en modo TEST).
    deleted_files = 0
    print("\n2. Archivos en Canvas Files conservados por seguridad.")
...
def crear_modulo_solemne(numero_solemne, html_path=None, course_id=None, 
    ...
    Args:
        ...
        course_id: ID del curso en Canvas (si es None, usa el ID del usuario seleccionado)
        fecha_inicio: Fecha de disponibilidad en formato "DD-MM-YYYY HH:MM"
        fecha_hasta: Fecha de entrega en formato "DD-MM-YYYY HH:MM"
...
def crear_modulo_examen(html_path=None, course_id=None, 
    ...
    Ejemplo:
        Canvas_Key.crear_modulo_examen(
            fecha_inicio="DD-MM-YYYY HH:MM",
            fecha_hasta="DD-MM-YYYY HH:MM",
        )
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
...
def mover_modulo(module_id, nueva_posicion=None, nuevo_nombre=None, publicado=None, course_id=None):
    ...
    Ejemplos:
        # Solo mover posición
        Canvas_Key.mover_modulo(module_id=123456, nueva_posicion=10)
        
        # Cambiar nombre
        Canvas_Key.mover_modulo(module_id=123456, nuevo_nombre="Evaluaciones")
        
        # Cambiar estado de publicación
        Canvas_Key.mover_modulo(module_id=123456, publicado=True)
        
        # Cambiar múltiples propiedades
        Canvas_Key.mover_modulo(module_id=123456, nueva_posicion=5, nuevo_nombre="Evaluaciones", publicado=True)
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
        print("💡 Primero ejecuta: Canvas_Key.select_user('TuNombre')")
...
def ver_contenido_modulo(module_id, course_id=None):
    ...
    Ejemplo:
        Canvas_Key.ver_contenido_modulo(module_id=123456)
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
        print("💡 Primero ejecuta: Canvas_Key.select_user('TuNombre')")
...
def mover_item_modulo(module_id, item_id, nueva_posicion, course_id=None):
    ...
    Ejemplo:
        Canvas_Key.mover_item_modulo(module_id=123456, item_id=12345, nueva_posicion=3)
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
        print("💡 Primero ejecuta: Canvas_Key.select_user('TuNombre')")
...
def modificar_item_modulo(module_id, item_id, nuevo_titulo=None, publicado=None, indent=None, course_id=None):
    ...
    Ejemplos:
        # Cambiar título
        Canvas_Key.modificar_item_modulo(module_id=123456, item_id=12345, nuevo_titulo="Nuevo nombre")
        
        # Cambiar nivel de sangría
        Canvas_Key.modificar_item_modulo(module_id=123456, item_id=12345, indent=2)
        
        # Cambiar múltiples propiedades
        Canvas_Key.modificar_item_modulo(module_id=123456, item_id=12345, nuevo_titulo="Archivo", publicado=True, indent=1)
    """
    if canvas is None:
        print("❌ Error: No hay conexión activa con Canvas")
        print("💡 Primero ejecuta: Canvas_Key.select_user('TuNombre')")
